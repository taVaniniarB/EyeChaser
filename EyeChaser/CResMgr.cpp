...
*/
// 메모리에서 IStream 생성
IStream* CResMgr::CreateStreamOnResource(void* pResourceData, DWORD dwResourceSize)
{
	...
	if (hGlobal)
	{
		...
		if (pData)
		{
			CopyMemory(pData, pResourceData, dwResourceSize);
...
Image* CResMgr::LoadImageFromMemory(const void* data, size_t size)
{
	IStream* stream = CreateStreamOnResource(const_cast<void*>(data), static_cast<DWORD>(size));
	if (stream)
	{
